...
We also developed patterns to be put on the walls with fixed feature density (Bernoulli distributions of features). The pattern can be created using the `Bernoulli_Textures.py` python scripts in which the density of the features can be set manually. Then the new pattern must be added to the Gazebo materials, the "pattern.png" generated by the python script has to be saved in `/usr/share/gazebo-x/media/materials/textures`. To use the new material in Gazebo, a "pattern.material" file must be created and saved in `/usr/share/gazebo-x/media/materials/scripts`. An example of "pattern.material" is:
```
material BernoulliMix/One
 {
    technique
    {
       pass
